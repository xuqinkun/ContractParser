...
def extract_contract(pages={}, filename=""):
    ...
    _first_dir, _parent_dir, _filename = _parse_dir(root, filename)
...
def write_excel(out_path, type,  contract_list):
    ...
    for _contract in contract_list:
        ...
        if len(_form.keys()) > 0:
            with pd.ExcelWriter(order_path) as writer:
                ...
                writer.save()
    print("Write %s succeed" % excel_filename)
